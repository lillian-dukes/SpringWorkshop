...
```{r GitHub-account}
library(usethis)
use_git_config(user.name="lillian-dukes", user.email="lillian.dukes@duke.edu")
```
...
```{r view-clarity}
#how to access a single column of data; produces a vector in r
...
diamonds <- diamonds%>%
  mutate(pricepermm = price / total)


```

```{r graph-of-something-interesting}
ggplot(data = diamonds, aes(x = clarity, y = pricepermm)) +
  geom_line() +
  geom_point()

```
```{r lily-graph-2}
ggplot(data = diamonds, aes(x = total, y = price, color = pricepermm)) + 
  geom_line() + 
  geom_point()
```
...
```{r try-averaging-diamond-prices-2}
diamonds %>%
  group_by(color) %>%
